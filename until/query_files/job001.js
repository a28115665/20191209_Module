	...
	switch(pQueryname){
		...
		case "SelectItemListForEx0MX3":
			...
			_SQLCommand += "SELECT BLFO_TRACK, \
								...
								PG_MOVED, \
								OUT_IL.*, \
								CASE WHEN ROW_NUMBER() OVER(PARTITION BY IL_MERGENO ORDER BY IL_FINALCOST DESC) = 1 \
								THEN( \
							...
							AND IL_SEQ = @IL_SEQ \
							AND IL_MERGENO IS NOT NULL \
							AND IL_BAGNOEX_NOREPEAT IS NOT NULL ";
			...
			_SQLCommand += "ORDER BY IL_MERGENO, IL_NEWSMALLNO ";
		...
		case "SelectItemListForEx12":
			_SQLCommand += "SELECT BLFO_TRACK,  \
									...
									/*因上稅規則所以拿掉此規則 CASE WHEN V_ITEM_LIST_EXIST_ITEM.IL_G1 = 'Y' THEN IL_GETNO ELSE IL_EXNO END AS 'IL_GETNO_X2',*/ \
									/*因實名制拿掉此規則 CASE WHEN V_ITEM_LIST_EXIST_ITEM.IL_G1 = '上稅' THEN IL_GETNO ELSE '' END AS 'IL_GETNO_X2',*/ \
