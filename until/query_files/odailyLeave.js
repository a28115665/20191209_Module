	...
	switch(pQueryname){
		case "SelectUserLeavebyGrade":
			_SQLCommand += "SELECT U_ID, \
								   ...
								   U_GRADE, \
								   UD_DEPT, \
								   CASE WHEN DL.O_DL_ID IS NOT NULL THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END AS 'O_DL_IS_LEAVE' \
							FROM USER_DEPT \
							JOIN USER_INFO ON U_ID = UD_ID \
							LEFT JOIN O_DAILY_LEAVE DL ON DL.O_DL_ID = UD_ID AND O_DL_DEPT = UD_DEPT \
							WHERE U_STS = 0 ";
